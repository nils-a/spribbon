...
if (DM || E) {
    d.write('<img src="' + imagesPath + '0.gif" id="PI" name="PI" width="32" height="30">')
...
function B(Q) {
    ...
    if (q == Q && I) {
        ...
        if (mv(q, Q, chessPromotion, y)) {
            O(p, 0)
            d.onmousemove = null
            if (A) Ic.top = Ic.left = '0px'
            I = 0
            if (!J) {
                var t = Z(chessDifficulty, M, 0, BE, BE, Al, Bt, K)
                mv(t[1], t[2], 0)
            } 
        } 
    } 
...
function Df(x) { d.getElementById('chessMoves').value += '\n ' + x }
function Rf(a) {
    P = a
    for (var z = 0; z < BE; z++) if (R[z] < 16) O(z, R[z], 1)
    if (P != M) {
        Tt = Z(chessDifficulty, M, 0, BE, BE, Al, Bt, K)
        mv(Tt[1], Tt[2], 0)
    } 
}
function Gb() {
...
function dr(e) {
    ...
    Ic.top = (e.clientY - 4 - 44) + 'px'
}
function O(x, y, z) {
    if ((A || x != 'PI') && z) x = "i" + (P ? 119 - x : x)
    d.images[x].src = imagesPath + y + '.gif'
}
...
for (y = 90; y > 10; y -= 10) {
    ...
    for (x = 0; x < 10; x++) {
        z = y + x
        if (x && x < 9) { h += ('<td class="' + (x + (y / 10) & 1 ? 'b' : 'w') + '"><a href="#" onclick="B(P?119-' + z + ':' + z + '); return false"><img src="' + imagesPath + '0.gif" width="1" height="32" border="0"><img src="' + imagesPath + '0.gif" width="32" height="30" name="i' + z + '" border="0"><img src="' + imagesPath + '0.gif" width="1" height="32" border="0"></a></td>\n') } 
    } h += '</tr>\n'
} h += '</table>'
d.write(h)
Rf(0)